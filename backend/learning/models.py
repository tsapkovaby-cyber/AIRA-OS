...
class Student:
    id: str = field(default_factory=new_id); created_at: datetime = field(default_factory=now)
@dataclass(slots=True)
class LearningGoal:
    id: str; description: str; target: str|None=None
@dataclass(slots=True)
...
@dataclass(slots=True)
class Exercise:
    id: str; title: str; kind: str; topic: str|None=None
@dataclass(slots=True)
class Assessment:
    id: str; title: str; exercise_ids: list[str]=field(default_factory=list); passing_score: float=.7
@dataclass(slots=True)
class Lesson:
...
@dataclass(slots=True)
class CourseModule:
    id: str; title: str; lessons: list[Lesson]=field(default_factory=list)
@dataclass(slots=True)
class Course:
...
@dataclass(slots=True)
class LearningPath:
    student_id: str; course_id: str; lesson_ids: list[str]
@dataclass(slots=True)
class Enrollment:
...
class LessonProgress:
    student_id: str; course_id: str; lesson_id: str; status: LearningStatus
@dataclass(slots=True)
...
@dataclass(slots=True)
class TutorSession:
    id: str; student_id: str; mode: str; lesson_id: str|None=None; created_at: datetime=field(default_factory=now)
