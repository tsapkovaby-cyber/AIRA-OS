...
class LearningPlatformService:
    def __init__(self, *, tutor: TutorPort | None = None, memory: LearningMemoryPort | None = None) -> None:
        self.students: dict[str, Student] = {}; self.profiles: dict[str, LearningProfile] = {}; self.courses: dict[str, Course] = {}
        self.enrollments: dict[tuple[str, str], Enrollment] = {}; self.results: list[ExerciseResult] = []
        self.tutor = tutor or FakeTutor(); self.memory = memory or InMemoryLearningMemory()
    ...
    def enroll(self, student_id: str, course_id: str) -> Enrollment:
        ...
        enrollment = Enrollment(id=new_id(), student_id=student_id, course_id=course_id); self.enrollments[key] = enrollment; return enrollment
    ...
    def get_learning_path(self, student_id: str, course_id: str) -> LearningPath:
        ...
        available = [l.id for l in course.ordered_lessons() if l.id not in enrollment.completed_lesson_ids and set(l.prerequisite_lesson_ids).issubset(enrollment.completed_lesson_ids)]
        return LearningPath(student_id, course_id, available)
    ...
    def start_lesson(self, student_id: str, course_id: str, lesson_id: str) -> Enrollment:
        ...
        enrollment.started_lesson_ids.add(lesson_id); enrollment.status = LearningStatus.IN_PROGRESS; return enrollment
    ...
    def complete_lesson(self, student_id: str, course_id: str, lesson_id: str) -> Enrollment:
        ...
        self.memory.remember(student_id, "completed_lessons", lesson_id); return enrollment
    ...
    def submit_exercise_result(self, result: ExerciseResult) -> ExerciseResult:
        ...
        if not any(e.id == result.exercise_id for e in lesson.exercises): raise NotFound(result.exercise_id)
    ...
    def progress(self, student_id: str, course_id: str) -> CourseProgress:
        ...
        return CourseProgress(course_id, len(enrollment.completed_lesson_ids), len(lessons), round((len(enrollment.completed_lesson_ids)/len(lessons)*100) if lessons else 0, 2), sorted(k for k,v in averages.items() if v >= .8), sorted(k for k,v in averages.items() if v < .6))
