from aira_os.guardian_engine import ClaimType, EvidenceRecord, GuardianEngine, ReviewResult, RiskCategory, RiskLevel
...
def test_evidence_validation_requires_sources():
    evidence = [EvidenceRecord(claim="Unsupported claim", claim_type=ClaimType.FACT, primary_source=None, secondary_source=None, publication_date=None, confidence=0.7, verification_status="Unverified")]
...
def test_approval_workflow_requires_founder_for_brand_domain():
    review = GuardianEngine().review_workflow(workflow="Public brand announcement", review_type="Brand Validation", evidence=complete_evidence(), constitution_principles=complete_principles(), risk_categories={RiskCategory.BRAND}, governed_domains={"brand"}, has_publication=True)
...
def test_incident_created_when_guardian_blocks_publication():
    engine = GuardianEngine()
    review = engine.review_workflow(workflow="Legal public statement", review_type="Legal Review", evidence=complete_evidence(), constitution_principles=complete_principles(), risk_categories={RiskCategory.LEGAL}, governed_domains={"legal"}, has_publication=True)
...
def test_policy_enforcement_blocks_publication_without_evidence():
    review = GuardianEngine().review_workflow(workflow="Unsourced publication", review_type="Evidence Validation", evidence=[], constitution_principles=complete_principles(), risk_categories={RiskCategory.EDUCATIONAL}, governed_domains=set(), has_publication=True)
...
def test_transparency_report_contains_required_fields():
    engine = GuardianEngine()
    review = engine.review_workflow(workflow="Educational content", review_type="Transparency Report", evidence=complete_evidence(), constitution_principles=complete_principles(), risk_categories={RiskCategory.EDUCATIONAL}, governed_domains=set(), has_publication=True)
