...
from backend.learning_api import LearningPlatformAPI, APIError
...
def test_logout_invalidates_api_session():
    ...
    with pytest.raises(Exception):api.list_courses(token)
...
def test_accounts_cannot_address_another_student_through_api():
    api,accounts,learning=platform();one=api.register("one@example.com","correct horse battery staple");two=api.register("two@example.com","correct horse battery staple");token=api.login("one@example.com","correct horse battery staple")["session_token"]
    assert accounts.get_primary_student(api.current_account(token).id).id==one["student_id"] and one["student_id"]!=two["student_id"]
