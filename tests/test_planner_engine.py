...
def test_plan_creation_initializes_workflow_and_tasks():
    ...
    goal = engine.create_goal("Build Planner Engine", "Design the Planner Engine architecture", GoalType.DEVELOPMENT, PlanningLevel.INITIATIVE)
...
def test_non_founder_cannot_approve_plan():
    engine = PlannerEngine()
    goal = engine.create_goal("Research Topic", "Produce research plan", GoalType.RESEARCH, PlanningLevel.PROJECT)
    plan = engine.generate_plan(goal)
    engine.request_approval(plan.plan_id)
    with pytest.raises(PermissionError, match="Founder"):
        engine.approve_plan(plan.plan_id, actor="agent")
