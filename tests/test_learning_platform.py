...
from backend.learning.service import DuplicateEnrollment, LearningPlatformService, PrerequisiteNotMet
...
def test_profile_course_enrollment_and_duplicate_guard():
    ...
    with pytest.raises(DuplicateEnrollment): service.enroll(student.id, course.id)
...
def test_student_isolation_and_tutor_memory_boundaries():
    ...
    with pytest.raises(Exception): service.progress(other.id, course.id)
