...
def setup_platform():
    service = LearningPlatformService(); student = service.create_student(); course = service.create_course(conversational_english_a1()); enrollment = service.enroll(student.id, course.id)
...
def test_profile_course_enrollment_and_duplicate_guard():
    ...
    profile = service.create_or_update_learning_profile(LearningProfile(student.id, native_language="Russian", target_languages=["English"], current_level="A1", target_level="B1"))
    assert service.get_student(student.id) is student and service.get_course(course.id) is course and profile.student_id == student.id
...
def test_lesson_prerequisites_progress_path_and_recommendation():
    service, student, course, _ = setup_platform(); first, second = course.ordered_lessons()
    assert service.get_next_recommended_lesson(student.id, course.id).id == first.id
    assert service.get_learning_path(student.id, course.id).lesson_ids == [first.id]
    with pytest.raises(PrerequisiteNotMet): service.start_lesson(student.id, course.id, second.id)
    service.start_lesson(student.id, course.id, first.id); service.complete_lesson(student.id, course.id, first.id)
    assert service.get_next_recommended_lesson(student.id, course.id).id == second.id
    assert service.get_learning_path(student.id, course.id).lesson_ids == [second.id]
    assert service.get_course_progress(student.id, course.id).completion_percentage == 50.0
...
def test_exercise_results_derive_strengths_and_weaknesses():
    ...
    progress = service.progress(student.id, course.id)
    assert progress.strengths == ["greetings"] and progress.weaknesses == ["introductions"]
...
def test_reference_course_is_subject_generic_platform_data():
    ...
    assert course.subject == "language" and course.language == "English" and len(course.modules) == 1 and len(course.ordered_lessons()) == 2
